...
Ny = 6;                                                                                                                                                                                                                                                                                                                                                                             
Nu = 6; 
file_regions  = ['regions_double_int_mpt3_', num2str(N), '.mat'];
% file_regions  = ['regions_example2_kd_tree_mpt3_', num2str(N), '.mat'];
% file_regions = ['regions_quad_int_mpt3_', '_Ny_', num2str(Ny), '_Nu_',  num2str(Nu), '.mat'];
...
for i = 1:size(ineq_list,1)
    for j = 1:size(Regions,1)
        result = side_ineq_region(ineq_list(i,:), Regions(j,:), solver_opt);
...
while isempty(unex_node) == 0 && it < it_max 
    ...
    if size(controls_remaining,1) < 5 % Original
        for i = 1:size(nodes{index_node,4},1)
...
fprintf('Time Inequations: %.3f   Time BST: %.3f   Total Time: %.3f\nNumber Ineq: %d   Number Nodes: %d\nNumber Leaf: %d   Depth: %d\n\n', time_ineq, time_bst, time_ineq + time_bst, size(ineq_list,1), size(nodes,1), number_leafs, max_depth);

%%

% name_file = ['bst_original_double_int_mpt3_', num2str(N), '.mat'];
% name_file = ['bst_original_example2_kd_tree_mpt3_', num2str(N), '.mat'];
% name_file = ['bst_original_quad_integrator_mpt3_', '_Ny_', num2str(Ny), '_Nu_',  num2str(Nu), '.mat'];
% name_file = ['bst_original_quadruple_tank_mpt3_', '_Ny_', num2str(Ny), '_Nu_',  num2str(Nu), '.mat'];
